...
/*-----------------------------------------------------------------------------------
 * The filter2d implementation in this file is sub-optimal but functional. The next
 * release will have a more perfomant version.
 *-----------------------------------------------------------------------------------
 */
...
/* Parallelization factor for 32b operations (8x mults) */
#define PARALLEL_FACTOR_32b 	8
/* SRS shift used can be increased if input data likewise adjusted) */
#define SRS_SHIFT 		0
...
int32_t kernel_coeff[16] = {1,2,1,0,0,0,-1,-2,-1};
int image_width = IMAGE_WIDTH;
int image_height = IMAGE_HEIGHT;
int stride = image_width;
int32 ROW1[IMAGE_WIDTH];
int32 ROW2[IMAGE_WIDTH];
int32 ROW3[IMAGE_WIDTH];
 ...
 *               ^              ^                   ^              ^             ^                   ^             ^         
 *        (accumulator) (single large buffer) (start x index) (x offsets) (coefficient buffer) (start z index) (z offsets)
 *                                                            (per lane )                                      (per lane )
...
void filter2D( input_stream_int32 * restrict img_in, output_stream_int32 * restrict img_out)
{
    ...
    int32_t const255[16] = {255, 255, 255, 255, 255, 255, 255, 255};
    v16int32 * restrict val255_ptr = (v16int32 *)const255;
    v16int32 val255 = *(val255_ptr);
    v16int32 val0;
    v16int32 data_con;
    v16int32 data_max;
    v16int32 data_min;
    ...
    v8int32 * restrict ptr0;
    v8int32 * restrict ptr1;
    v8int32 * restrict ptr2;
    //v8int32 * restrict ptr_out = ptr_img_out;
    val0 = null_v16int32();
	...
	iter++;
   	//printf(" iter count = %d\n" , iter);
    	...
    	if(flag_1 == 0) 
		{
            ...
            {
				ROW1[i] = readincr(img_in);
			}
		
			for(int i = 0; i < stride; i++) 
            chess_prepare_for_pipelining
            {
				ROW2[i] = readincr(img_in);
			}
			for(int i = 0; i < stride; i++) 
            chess_prepare_for_pipelining
            {
				ROW3[i] = readincr(img_in);
			}
			load_new_data = 1;
			
		}	
		else if (load_new_data == 1)
		{
			ptr0 = (v8int32 *)ROW2;
			ptr1 = (v8int32 *)ROW3;
			ptr2 = (v8int32 *)ROW1;
    ...
    if (flag_1 == 0)
    {
        ...
        //*(ptr_out++) = data_out;
        data_con = xset_w(0, data_out);
        // round down negative values to 0
	data_max = max16(data_con, 0, 0x76543210, 0, val0, 0, 0x76543210, 0);  
	data_min = min16(data_max, 0, 0x76543210, 0, val255, 0, 0x76543210, 0);  

        tmp_out0 = ext_v(data_min, 0);
        tmp_out1 = ext_v(data_min, 1);
        
        writeincr_v4(img_out,tmp_out0);
        writeincr_v4(img_out,tmp_out1);
