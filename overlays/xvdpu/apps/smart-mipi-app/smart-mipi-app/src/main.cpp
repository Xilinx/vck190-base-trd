...
static const struct
{
    ...
    {0x0098090e, 140 },    //control 0x0098090e `AR0231 Red Balance' min 0 max 2047 step 1 default 128 current 140.
    {0x0098090f, 500 },    //control 0x0098090e `AR0231 Blue Balance' min 0 max 2047 step 1 default 128 current 140.
    {0x00980911, 1000},    //control 0x00980911 `AR0231 Exposure' min 16 max 1339 step 1 default 821 current 1000.
    {0x00980913, 800 },    //control 0x00980913 `AR0231 Digital Gain' min 0 max 2047 step 1 default 512 current 800.
    {0x00980924, 180 },    //control 0x00980924 `AR0231 Green Balance' min 0 max 2047 step 1 default 145 current 180.`
};

// quad-mipi sensor v4l2 sub-device
static const char* subdev[] = {
    "/dev/v4l-subdev14",
    "/dev/v4l-subdev13",
    "/dev/v4l-subdev12",
    "/dev/v4l-subdev11",
...
PipelineInfo info = {
    ...
    1,
    (gchar *)"yolov3",
    {(gchar *)"refinedet", (gchar *)"facedetect", (gchar *)"ssd", (gchar *)"yolov3"},
...
int main(int argc, char *argv[])
{
    ...
    if (!info.single) {
         for (int m = 0; m < ARRAY_SIZE(subdev); ++m) {
             for (n = 0; n < ARRAY_SIZE (default_ctrls); ++n) {
                 ret = v4l2_set_ctrl (subdev[m], default_ctrls[n].id, default_ctrls[n].val);
                 if (ret < 0)
                     continue;
            }
        }
